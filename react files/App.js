...
const App = () => {
  ...
  const [providers, setProviders] = useState([]);  // Added providers state
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8080/api/provider")
      .then((response) => setProviders(response.data))  // Now using setProviders
        ...
        <div className="col-md-6">
          ...
          <ul className="list-group">
            {providers.map((provider) => (
              <li key={provider.providerID} className="list-group-item">
                {provider.name}
              </li>
            ))}
          </ul>
